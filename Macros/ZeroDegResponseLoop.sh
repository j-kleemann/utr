...
echo Preparing to loop-simulate the zero degree detector response
cd "$(dirname "$0")/.."
cmake -DUSE_GPS=ON -DDETECTOR_CONSTRUCTION=150Nd_152Sm_775_784_ZeroDeg     .  || exit
make -j$(nproc --all) || exit

outputdir="outputdir"
threads=$(nproc --all)
for sim in "Macros/150Nd_ZeroDegResponse_Loop.mac" ; do
